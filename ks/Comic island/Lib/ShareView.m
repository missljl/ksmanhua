...
-(void)iskehuduan{
    BOOL isweixin = [JSHAREService isWeChatInstalled];
    if (isweixin== NO) {
        [self.currentContentSupportPlatform removeObjectsInArray:@[@(JSHAREPlatformWechatSession),@(JSHAREPlatformWechatTimeLine),@(JSHAREPlatformWechatFavourite)]];
    }
    BOOL isweibo = [JSHAREService isSinaWeiBoInstalled];
    if (isweibo ==NO) {
         [self.currentContentSupportPlatform removeObjectsInArray:@[@(JSHAREPlatformSinaWeibo)]];
        
    }
    BOOL isqq = [JSHAREService isQQInstalled];
    if (isqq ==NO) {
        
        [self.currentContentSupportPlatform removeObjectsInArray:@[@(JSHAREPlatformQQ),
                                                                    @(JSHAREPlatformQzone)]];
    }
